...
def _drawing_key(obs: dict) -> dict:
    ...
    mapping = {
        ...
        "tile_mode": _pack("Database.TileMode。true=当前在模型空间；补全前字段目录未单列此键，数据里有。", "七张全 true。", "与 layouts 的 Model 实体数一起证明内容在模型空间，图纸空间空。", COMBO, "Teigha Database.TileMode"),
        "measurement": _pack("Database.Measurement=Metric。", "公制绘图。", "与 insunits=Undefined 组合：单位仍要约定 mm，不能只信这个枚举。", COMBO, "Database.Measurement"),
        "insunits": _pack("插入单位。七张 Undefined；补全前字段目录未单列。", "单独不能换算。", "铁芯毫米判定必须外部约定 1 单位=1mm，否则 measurement 不能当 mm。", COMBO, "Database.Insunits"),
