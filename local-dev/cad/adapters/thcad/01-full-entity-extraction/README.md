...
- 将最大绘图区图框及四边句柄、全部模型空间文字、04 BOM、05 技术要求和 `PC_TITLE_BLOCK` 包围盒适配给 13；04/11/13 随即进入 21，对同定义块实例与全部序号指向做候选口径的覆盖对账。13 的 scope 结果再送入 14，由 15 形成保守身份图，12/13/15 进入 16 形成视图内轮廓候选，17 整理接口与邻接证据，18 再把 09 尺寸 occurrence 绑定到这些结构事实并作量值筛查；19 最后把 04/05/08/11/13/16/17/18 投影为当前图纸的稳定语义快照，20 从中压出可供项目汇聚的单图 observation，并补入块表已声明的外参路径。Adapter 不在单图内伪造项目图，也不把 21 候选判成漏标、数量错误或 BOM 缺陷。
