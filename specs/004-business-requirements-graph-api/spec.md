...
状态：历史实现，当前无活动数据物化。旧 SQLite 已按 016 清理；本规格保留供以后决定是否重建。

## 背景

`shenbian.client_requirements.curated.v1` 由 curated JSONL 物化到本机 SQLite（见 [`specs/016-local-sqlite-query-layer/`](../016-local-sqlite-query-layer/)）。前端需要真实接口读取 240 个业务需求、249 条关系和节点详情，不能依赖 Mock，也不能直连 SQLite 或 PostgreSQL。
...
- **R3** 数据只从本机 SQLite 查询层读取，路径由根 `.env` 的 `BUSINESS_REQUIREMENTS_SQLITE` 给出；前端不读取 JSONL 或数据库。不得再把该数据集写入 `DATABASE_URL`。
...
- **R6** 数据集、视图和需求不存在时分别返回稳定的 `404` 错误码；SQLite 文件缺失或查询层不可用时返回不泄露路径的 `503`。
...
1. 真实本机 SQLite 请求返回 `nodes=240`、`edges=249`、`atomic_requirements=201`、`level_one_requirements=20`。
