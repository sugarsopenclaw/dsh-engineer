...
状态：历史数据集规格，当前不物化到 FastAPI。旧 SQLite 已按 016 清理。

## 目标

把已经人工确认的 THCAD V24 与 AutoCAD 2024 原子库存作为同一种 `CapabilityAtom` 对象物化到本机 SQLite（见 [`specs/016-local-sqlite-query-layer/`](../016-local-sqlite-query-layer/)），并通过现有 `/api/v1/cad-capabilities` 契约查询。每个宿主的观测保持独立行，不按名称或猜测合并原子。不得再写入 `DATABASE_URL`。
...
- **R2 同一对象类型**：两个宿主都写入本机 `cad-capabilities.sqlite` 的 `capability_atoms` 和 `capability_inventories`，由 `dataset_id` 区分数据集版本，不新建宿主专用表。
- **R3 旧版保护**：生成和导入 v2 不得删除、覆盖或改写 v1 文件与已物化的 v1 行。
...
- **R8 图投影传输**：不得为每次冷启动逐行扫描数万条宽记录。允许由同一 dataset 内容在本机确定性生成 gzip 最小投影并随 dataset 级联失效；投影不是新原子或语义关系。
...
2. 本机 SQLite 可同时存在 v1 的 79,549 条和 v2 的 334,049 条原子；默认 API 读 v2。
