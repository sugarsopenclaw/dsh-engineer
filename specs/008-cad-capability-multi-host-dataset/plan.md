...
4. 用本机 SQLite 导入器写入同一组查询表；只替换相同 `dataset_id` 的行。不得再写入 `DATABASE_URL`。
5. v2 验收后切换 API 默认数据集，保留显式 v1 查询。
6. 入库时在本机生成按“数据集 + 技术面 + 宿主”分片的 gzip 图投影；批量 API 冷启动读取压缩投影，列表和详情仍以原子表为准。
...
- `data/datasets/local/cad-capabilities.sqlite` 的 `capability_graph_snapshots`：与 dataset 同生命周期的本机 gzip 图投影，不进 git、不写 `DATABASE_URL`；
