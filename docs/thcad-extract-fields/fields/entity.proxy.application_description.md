# 字段：application_description

- **字段 ID**：`entity.proxy.application_description`
- **JSON 路径**：`entities.jsonl / proxy.application_description`
...
- 来源文件：`entities.jsonl`
- JSON 路径：`entities.jsonl / proxy.application_description`
- 作用域：`entity.proxy`（同名键按表区分，例如 `layer.handle` ≠ `entity.handle`）
